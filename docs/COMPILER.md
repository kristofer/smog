...
```
Source Code → Lexer → Parser → AST → **COMPILER** → Bytecode → [.sg file] → VM
```
...
5. **Serialization**: (Optional) Save bytecode to .sg files for faster loading
6. **Optimization**: (Future) Optimize generated bytecode for efficiency
...
- [Bytecode Format Guide](BYTECODE_FORMAT.md) - .sg file format and CLI usage
...
The compiler generates bytecode that can be:
1. **Executed immediately** - Pass directly to the VM
2. **Saved to .sg files** - Binary format for distribution
...
defer file.Close()
bytecode, err := bytecode.Decode(file)
...
- Skip parsing and compilation (5-50x faster startup)
- Distribute programs without source code
- Enable multi-file module systems
- Reduce memory footprint during loading

See [Bytecode Format Guide](BYTECODE_FORMAT.md) for complete details.

## Summary

The Smog compiler transforms high-level object-oriented code into efficient bytecode instructions. It manages variables, builds constant pools, and handles the complexities of message passing and closures. The generated bytecode can be executed immediately or saved to .sg files for faster loading. Understanding the compiler is key to understanding how Smog programs execute efficiently on the virtual machine.
